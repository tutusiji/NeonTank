...
import { GRID_SIZE, LEVEL_1, COLORS, GAME_SPEED, BULLET_SPEED, TANK_COOLDOWN } from '../constants';
import { Direction, TileType, Position, Tank, Bullet, Explosion, GameState } from '../types';
import { TankModel, BrickWall, SteelWall, WaterTile, BaseEagle, BulletMesh, ExplosionMesh, gridToWorld } from './Models';
...
const GameLoop: React.FC<GameEngineProps> = ({ gameState, setGameState, onScore }) => {
  ...
  const enemiesRef = useRef<Tank[]>([
    { id: 'e1', position: { x: 0, z: 0 }, direction: Direction.DOWN, active: true, type: 'enemy', cooldown: 50, hp: 1 },
    { id: 'e2', position: { x: 6, z: 0 }, direction: Direction.DOWN, active: true, type: 'enemy', cooldown: 100, hp: 1 },
    { id: 'e3', position: { x: 12, z: 0 }, direction: Direction.DOWN, active: true, type: 'enemy', cooldown: 150, hp: 1 },
  ]);
  ...
  useFrame((state, delta) => {
    ...
    if ((keys.current['Space'] || keys.current['Enter']) && player.cooldown <= 0 && player.active) {
    ...
    enemiesRef.current.forEach(enemy => {
      ...
      switch (enemy.direction) {
        case Direction.UP: enemyIntended.z -= moveAmt; break;
        case Direction.DOWN: enemyIntended.z += moveAmt; break;
        case Direction.LEFT: enemyIntended.x -= moveAmt; break;
        case Direction.RIGHT: enemyIntended.x += moveAmt; break;
      }
    ...
    bulletsRef.current.forEach(bullet => {
      ...
      switch (bullet.direction) {
        case Direction.UP: bullet.position.z -= BULLET_SPEED; break;
        case Direction.DOWN: bullet.position.z += BULLET_SPEED; break;
        case Direction.LEFT: bullet.position.x -= BULLET_SPEED; break;
        case Direction.RIGHT: bullet.position.x += BULLET_SPEED; break;
      }
      ...
      if (gx >= 0 && gx < GRID_SIZE && gz >= 0 && gz < GRID_SIZE) {
        ...
        if (tile === TileType.BRICK) {
          ...
          bullet.active = false;
          mapChanged = true;
          explosionsRef.current.push({ id: Math.random().toString(), position: { x: gx, z: gz }, scale: 1, createdAt: state.clock.elapsedTime });
        } else if (tile === TileType.STEEL) {
          bullet.active = false;
          explosionsRef.current.push({ id: Math.random().toString(), position: { x: gx, z: gz }, scale: 0.5, createdAt: state.clock.elapsedTime });
        } else if (tile === TileType.BASE) {
          bullet.active = false;
          map[gz][gx] = 0; 
          setGameState(prev => ({ ...prev, status: 'gameover' }));
           explosionsRef.current.push({ id: Math.random().toString(), position: { x: gx, z: gz }, scale: 3, createdAt: state.clock.elapsedTime });
        }
      } else {
        ...
        if (bullet.owner === 'player') {
           ...
           enemiesRef.current.forEach(enemy => {
             if (enemy.active && isColliding(bullet.position, enemy.position)) {
               ...
               bullet.active = false;
               onScore(100);
               explosionsRef.current.push({ id: Math.random().toString(), position: enemy.position, scale: 1.5, createdAt: state.clock.elapsedTime });
             }
           });
        } else {
          // Check player
          if (player.active && isColliding(bullet.position, player.position)) {
             ...
             bullet.active = false;
             setGameState(prev => ({ ...prev, status: 'gameover' }));
             explosionsRef.current.push({ id: Math.random().toString(), position: player.position, scale: 2, createdAt: state.clock.elapsedTime });
          }
        }
      }
    });
    ...
    explosionsRef.current = explosionsRef.current.filter(e => state.clock.elapsedTime - e.createdAt < 0.5);
  ...
  const mapElements = useMemo(() => {
    ...
    for (let z = 0; z < GRID_SIZE; z++) {
      for (let x = 0; x < GRID_SIZE; x++) {
        const tile = map[z][x];
        if (tile === TileType.BRICK) elements.push(<BrickWall key={`w-${x}-${z}`} x={x} z={z} />);
        if (tile === TileType.STEEL) elements.push(<SteelWall key={`s-${x}-${z}`} x={x} z={z} />);
        if (tile === TileType.WATER) elements.push(<WaterTile key={`wt-${x}-${z}`} x={x} z={z} />);
        if (tile === TileType.BASE) elements.push(<BaseEagle key={`b-${x}-${z}`} x={x} z={z} dead={false} />);
        if (z === 12 && x === 6 && tile === 0 && LEVEL_1[12][6] === TileType.BASE) {
             elements.push(<BaseEagle key={`b-${x}-${z}-dead`} x={x} z={z} dead={true} />);
        }
      }
    }
    elements.push(
      <mesh key="floor" rotation={[-Math.PI/2, 0, 0]} position={[0, 0, 0]} receiveShadow>
...
const GameCanvas: React.FC<GameEngineProps> = ({ gameState, setGameState, onScore }) => {
  return (
    <Canvas shadows camera={{ position: [0, 12, 12], fov: 35, near: 0.1, far: 100 }}>
      ...
      <GameLoop gameState={gameState} setGameState={setGameState} onScore={onScore} />
